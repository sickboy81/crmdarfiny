...
export function InviteMemberDialog({
  ...
  const EXPIRY_OPTIONS: { value: string; label: string }[] = [
    { value: '1', label: t('expiration1d') },
    { value: '7', label: t('expiration7d') },
    { value: '30', label: t('expiration30d') },
  ];

  const ROLE_DESCRIPTIONS: Record<InviteRole, string> = {
    admin: t('roleAdminDesc'),
    agent: t('roleAgentDesc'),
    viewer: t('roleViewerDesc'),
  };
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('failedToCreateInvite'));
        return;
      }
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">{t('inviteATeammate')}</DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('generateOneTimeLink')}
              </DialogDescription>
            </DialogHeader>
