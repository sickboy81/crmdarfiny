...
// EDITABLE_ROLES is created inside the component so labels can be
// translated via the `t` function.
...
export function MembersTab() {
  ...
  const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
    { value: 'admin', label: t('roleAdmin'), hint: t('roleAdminDesc') },
    { value: 'agent', label: t('roleAgent'), hint: t('roleAgentDesc') },
    { value: 'viewer', label: t('roleViewer'), hint: t('roleViewerDesc') },
  ];

  const ROLE_LABELS: Record<AccountRole, string> = {
    owner: t('roleAdmin'), // closest match; owner is not editable
    admin: t('roleAdmin'),
    agent: t('roleAgent'),
    viewer: t('roleViewer'),
  };
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('failedToLoadMembers'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('failedToLoadInvitations'));
          return;
        }
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(t('roleUpdated', { name: member.full_name || '—', role: nextRole }));
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      toast.success(t('removedMember', { name: removingMember.full_name || '—' }));
      setMembers((prev) =>
                          ...
                          <Avatar className="size-9 shrink-0">
                            {member.avatar_url ? (
                              <AvatarImage
                                ...
                                alt={member.full_name || t('memberSingular')}
                              />
                            ) : null}
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {fmtDate(inv.created_at)} · {fmtExpiresIn(inv.expires_at, t)}
                        </p>
                      </div>
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {t('removeMemberConfirmation', { name: removingMember?.full_name || '—' })}
            </DialogDescription>
          </DialogHeader>
