...
export default async function handler(req: Request) {
  ...
  if (isImageRequest) {
    return new ImageResponse(
      (
        <div style= {{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#020617', padding: '40px' }}>
          <div style={ { display: 'flex', flexDirection: 'row', alignItems: 'center', backgroundColor: '#0f172a', border: '2px solid #1e293b', borderRadius: '48px', padding: '60px', width: '1100px', borderTop: '6px solid #22c55e' } }>
            <img src={ avatar } style = {{ width: '250px', height: '250px', borderRadius: '40px', border: '4px solid #22c55e', marginRight: '50px', objectFit: 'cover' }
} />
...
const ogImageUrl = `https://crm.darfinyavila.com.br/api/bio?type=image&v=${Date.now()}`;
